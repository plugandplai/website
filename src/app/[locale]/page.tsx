...
import { ArrowRight, Palette, Sparkles, Shield, Server, Lock, MessageSquare, Users, CheckCircle } from "lucide-react";
import Image from "next/image";
import { useTranslations } from 'next-intl';

const enterpriseFeatures = [
  { icon: Palette, titleKey: "customBranding.title", descKey: "customBranding.description" },
  { icon: Shield, titleKey: "enterpriseSecurity.title", descKey: "enterpriseSecurity.description" },
  { icon: Server, titleKey: "onPremDeployment.title", descKey: "onPremDeployment.description" },
  { icon: Lock, titleKey: "roleBasedAccess.title", descKey: "roleBasedAccess.description" },
  { icon: MessageSquare, titleKey: "multiChannel.title", descKey: "multiChannel.description" },
  { icon: Users, titleKey: "teamManagement.title", descKey: "teamManagement.description" },
];
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 stagger-children">
            {enterpriseFeatures.map((feature) => (
              <div key={feature.titleKey} className="glass-card p-6 rounded-xl group hover-lift">
                ...
                <h3 className="text-xl font-medium text-foreground mb-2">{t(`enterpriseFeatures.${feature.titleKey}`)}</h3>
                <p className="text-sm text-muted-foreground">{t(`enterpriseFeatures.${feature.descKey}`)}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="animate-slide-left" style={{ animationDelay: '0ms' }}>
              ...
              <ul className="space-y-4 stagger-children">
                {(t.raw('customerStory.trumpf.points') as string[]).map((point: string) => (
            ...
            <div className="glass-card p-8 rounded-2xl animate-slide-right" style={{ animationDelay: '150ms' }}>
              <Image src="/customer_dgki.png" alt="DGKI" width={600} height={400} className="aspect-video object-contain rounded-xl w-full" />
            </div>
          </div>
        </div>
      </section>
