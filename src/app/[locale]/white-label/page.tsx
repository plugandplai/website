...
const features = [
  { icon: Palette, titleKey: "branding.title", descKey: "branding.description" },
  { icon: Globe, titleKey: "pricing.title", descKey: "pricing.description" },
  { icon: Mail, titleKey: "dashboard.title", descKey: "dashboard.description" },
  { icon: DollarSign, titleKey: "support.title", descKey: "support.description" },
];
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 stagger-children">
            {features.map((feature) => (
              <div key={feature.titleKey} className="glass-card p-6 rounded-xl group hover-lift">
                ...
                <h3 className="text-xl font-medium text-foreground mb-2">{t(`whiteLabel.features.${feature.titleKey}`)}</h3>
                <p className="text-sm text-muted-foreground">{t(`whiteLabel.features.${feature.descKey}`)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container-custom">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="glass-card p-8 rounded-2xl order-2 md:order-1 animate-slide-left" style={{ animationDelay: '0ms' }}>
              <Image src="/customer_flowvoice.png" alt="FlowVoice" width={600} height={400} className="w-full object-contain rounded-xl" />
            </div>
            <div className="order-1 md:order-2 animate-slide-right" style={{ animationDelay: '150ms' }}>
              ...
              <ul className="space-y-4 stagger-children">
                {(t.raw('whiteLabel.customerStory.points') as string[]).map((point: string) => (
