...
const components = [
  { icon: FileCode, titleKey: "react.title", descKey: "react.description" },
  { icon: FileCode, titleKey: "vue.title", descKey: "vue.description" },
  { icon: Package, titleKey: "wordpress.title", descKey: "wordpress.description" },
  { icon: Package, titleKey: "shopify.title", descKey: "shopify.description" },
];

const apiFeatures = [
  { icon: Code2, titleKey: "sdks.title", descKey: "sdks.description" },
  { icon: Webhook, titleKey: "webhooks.title", descKey: "webhooks.description" },
  { icon: Radio, titleKey: "streaming.title", descKey: "streaming.description" },
  { icon: Bot, titleKey: "multiAgent.title", descKey: "multiAgent.description" },
];
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 stagger-children">
            {components.map((item) => (
              <div key={item.titleKey} className="glass-card p-6 rounded-xl group hover-lift">
                ...
                <h3 className="text-xl font-medium text-foreground mb-2">{t(`developers.components.${item.titleKey}`)}</h3>
                <p className="text-sm text-muted-foreground">{t(`developers.components.${item.descKey}`)}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="animate-slide-left" style={{ animationDelay: '0ms' }}>
              ...
              <ul className="space-y-4 stagger-children">
                {(t.raw('developers.customerStory.points') as string[]).map((point: string) => (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 stagger-children">
            {apiFeatures.map((item) => (
              <div key={item.titleKey} className="glass-card p-6 rounded-xl group hover-lift">
                ...
                <h3 className="text-xl font-medium text-foreground mb-2">{t(`developers.apiFeatures.${item.titleKey}`)}</h3>
                <p className="text-sm text-muted-foreground">{t(`developers.apiFeatures.${item.descKey}`)}</p>
              </div>
            ))}
          </div>
        </div>
